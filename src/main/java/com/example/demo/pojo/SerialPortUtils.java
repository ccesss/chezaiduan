    ...
    public void readComm() throws Exception {
        try {
            ...
            while ((len = inputStream.read(readBuffer)) != -1) { // 直接获取到的数据
                ...
                if((data.contains("$GNRMC"))&(data.indexOf('*')>data.indexOf('$'))&(data.indexOf('G')==(data.indexOf('$')+1)))
                {
//                    System.out.println("data:" + data);
                    GNRMC = data.substring(data.indexOf('$'),data.indexOf("*")+3);
                    System.out.println(GNRMC);

                    String [] split=GNRMC.split(",");
                    if(split[2].equals("A")){
                        int end=GNRMC.indexOf("*");
                        int sum=0;
                        for(int i=1;i<end;i++) {
                            sum^=(int)GNRMC.charAt(i); }
                        sum %= 65536;
                        if(sum==Integer.parseInt(GNRMC.substring(GNRMC.indexOf("*") +1),16)) {
                            System.out.println("定位成功");
                            latAndLon latAndLon = new latAndLon();
                            String strLongitude = split[5];//经度
                            String strLatitude = split[3];//纬度

                            String nanbei = split[4];
                            String dongxi = split[6];
                            String vStr = split[7];
                            double latitude;
                            double longitude;
                            double v;
                            latitude = Double.valueOf(strLatitude)/100;
                            longitude = Double.valueOf(strLongitude)/100;
                            v = Double.valueOf(vStr)*1.852;
                            ...
                            latAndLon.setEw(dongxi);
                            latAndLon.setNs(nanbei);
                            latAndLon.setLat(strLatitude);
                            latAndLon.setLon(strLatitude);
                            latAndLon.setV(vStr);

                            System.out.println("纬度是：" + strLatitude);
                            System.out.println("经度是："+strLongitude);
                            System.out.println("速度是："+v);

                            //gpsMapper.updateGps(1,dongxi,nanbei,strLatitude,strLongitude,vStr);
                            //gpsMapper.updateGps(1,dongxi,nanbei,strLatitude,strLongitude,vStr);
                            //turnBack(latAndLon);
                            //return latAndLon;
...
//            }
    ...
    /*回调*/
    public latAndLon turnBack(latAndLon l){
        return l;
    }
}
